...
const resetPassword = async(req, res, next) => {
    try {
        ...
        if(!password || !confirmPassword){
            return next(new CustomError("Please enter password and confirm password", 400));
        }

        const encryptedToken = crypto.createHash("sha256").update(token).digest("hex");

        const user = await User.findOne({ forgotPasswordToken: encryptedToken, forgotPasswordTokenExpiry: { $gt: Date.now() } });
